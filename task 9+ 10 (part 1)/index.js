/*Task # 9 : Favorite Number: Store your favorite number in a variable. Then, using that variable, create a message
reveals your favorite number. Print that message.

Task # 10 (part 1 ) : Adding Comments: Choose two of the programs you’ve written, and add at least one comment to each. If you don’t
have anything specific to write because your programs are too simple at this point, just add your name and the
 current date at the top of each program file. Then write one sentence describing what the program does.*/
//Amber Asis
//tuesday , 29-may-2024
//store my favorite number in variable
const Favorite_Number = 4;
//reveaning my favorite number
const Message = (`MY favorite number is , ${Favorite_Number}`);
console.log(Message);
export {};
